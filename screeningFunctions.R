...
screenOffScale <- function(pid, time, timeBase="5 min"){
  ...
  timeBreaks <- seq(round(min(time,na.rm=TRUE), "hour"),
                    round(max(time,na.rm=TRUE), "hour"), timeBase) ## HL Added na.rm=TRUE on 12/12/2019
  ...
  offLow <- dfStats$timeCut[which(dfStats$mean < 0)]
...
screenRH <- function(pid, time, rh){
  ...
  timeBreaks <- seq(round(min(time,na.rm=TRUE), "hour"),
                    round(max(time,na.rm=TRUE), "hour"), timeBase) # HL added na.rm=TURE 12/12/2019
  df$timeCut <- cut(df$time, timeBreaks)
...
screenDrops <- function(pid, time, max_iter){
  ...
  timeBreaks <- seq(round(min(time,na.rm=TRUE), "min"), #ATS Added na.rm=TRUE 3/4/20
                    round(max(time,na.rm=TRUE), "min"), "5 sec") #ATS Added na.rm=TRUE 3/4/20
